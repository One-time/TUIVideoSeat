...
class TUIVideoSeatPresenter: NSObject {
    ...
    init(roomEngine: TUIRoomEngine, roomId: String) {
        ...
        currentUser.userInfo.userId = TUILogin.getUserID()
        currentUser.userInfo.userName = TUILogin.getNickName()
        currentUser.userInfo.avatarUrl = TUILogin.getFaceUrl()
        if currentUser.userInfo.userId == roomId {
